...
impl<'a> Decoder<'a> {
    ...
    /// Creates a new `Decoder` with the header_filter and all settings set to default values.
    pub fn new_with_header_filter(header_filter: fn(&[u8]) -> bool) -> Decoder<'a> {
        Decoder::with_header_filter(STATIC_TABLE, header_filter)
    }
    ...
    /// Creates a new `Decoder` with the given slice serving as its static
    /// table and the header filter.
    ///
    /// The slice should contain tuples where the tuple coordinates represent
    /// the header name and value, respectively.
    ///
    /// The header filter is used to filter headers that are not within the
    /// business scope, and the name and value of these headers are not written
    /// to the dynamic table, but in order for the dynamic table to be indexed
    /// correctly, a place is reserved for these headers in the dynamic table
    /// and replaced by "None"
    ///
    /// Note: in order for the final decoded content to match the encoding
    ///       (according to the standard, at least), this static table must be
    ///       the one defined in the HPACK spec.
    fn with_header_filter(
        static_table: StaticTable<'a>,
        header_filter: fn(&[u8]) -> bool,
    ) -> Decoder<'a> {
        Decoder {
            header_table: HeaderTable::with_header_filter(static_table, header_filter),
        }
    }
        ...
        F: FnMut(Cow<[u8]>, Cow<[u8]>),
    {
        ...
        while current_octet_index < buf.len() {
            ...
            let consumed = match FieldRepresentation::new(initial_octet) {
                FieldRepresentation::Indexed => {
                    let (name_value, consumed) = self.decode_indexed(buffer_leftover)?;
                    if let Some(nv) = name_value {
                        cb(Cow::Borrowed(nv.0), Cow::Borrowed(nv.1));
                    }

                    consumed
                }
                FieldRepresentation::LiteralWithIncrementalIndexing => {
                    let (name_value, consumed) = {
                        let (name_value, consumed) = self.decode_literal(buffer_leftover, true)?;
                        if let Some(nv) = name_value {
                            let (name, value) = nv;
                            ...
                            (Some((name.into_owned(), value.into_owned())), consumed)
                        } else {
                            (None, consumed)
                        }
                    };
                    ...
                    self.header_table.add_header(name_value);

                    consumed
                }
                FieldRepresentation::LiteralWithoutIndexing => {
                    let (name_value, consumed) = self.decode_literal(buffer_leftover, false)?;
    ...
    fn decode_indexed(&self, buf: &[u8]) -> Result<(Option<(&[u8], &[u8])>, usize), DecoderError> {
        ...
        let name_value = self.header_table.get_from_table(index)?;

        Ok((name_value, consumed))
    }
    ...
    fn decode_literal<'b>(
        ...
        buf: &'b [u8],
        index: bool,
    ) -> Result<(Option<(Cow<[u8]>, Cow<[u8]>)>, usize), DecoderError> {
        ...
        let name = if table_index == 0 {
            ...
            let name_value = self.header_table.get_from_table(table_index)?;
            if let Some(nv) = name_value {
                Cow::Borrowed(nv.0)
            } else {
                // If name is not in table, it is not expected_header, but value is still consume buffer
                let (_, value_len) = decode_string(&buf[consumed..])?;
                consumed += value_len;
...
mod tests {
    ...
    const USER_AGENT: &[u8] = b"user-agent";
    const REFERER: &[u8] = b"referer";

    /// Judge whether it is the expected header according to the incoming header
    const fn is_expected_header(header: &[u8]) -> bool {
        match header {
            METHOD | STATUS | HOST | AUTHORITY | PATH | CONTENT_TYPE | USER_AGENT | REFERER => true,
            _ => false,
        }
    }
    ...
    fn test_decode_fully_in_static_table() {
        let mut decoder = Decoder::new_with_header_filter(is_expected_header);
    ...
    fn test_decode_literal_name_in_dynamic() {
        ...
        }
        {
            ...
            let expected_table = vec![None, None];
            let actual = decoder.header_table.dynamic_table.to_vec();
            assert_eq!(actual, expected_table);
        }
    }
    ...
    fn test_request_sequence_no_huffman() {
        let mut decoder = Decoder::new_with_header_filter(is_expected_header);
        {
            ...
            assert_eq!(decoder.header_table.dynamic_table.len(), 1);
            let expected_table = vec![Some((b":authority".to_vec(), b"www.example.com".to_vec()))];
            let actual = decoder.header_table.dynamic_table.to_vec();
            assert_eq!(actual, expected_table);
        }
        {
            ...
            let expected_table = vec![
                None,
                Some((b":authority".to_vec(), b"www.example.com".to_vec())),
            ];
            let actual = decoder.header_table.dynamic_table.to_vec();
            assert_eq!(actual, expected_table);
        }
    ...
    fn response_sequence_no_huffman() {
        ...
        decoder.set_max_table_size(256);
        {
            ...
            let expected_table = vec![
                ...
                None,
                None,
                Some((b":status".to_vec(), b"302".to_vec())),
            ];
            let actual = decoder.header_table.dynamic_table.to_vec();
            assert_eq!(actual, expected_table);
        }
