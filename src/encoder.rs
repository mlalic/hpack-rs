    ...
    pub fn encode_header_into<W: io::Write>(
        ...
        match self.header_table.find_header(header) {
            None => {
                ...
                self.header_table
                    .add_header(Some((header.0.to_vec(), header.1.to_vec())));
            }
            Some((index, false)) => {
...
mod tests {
    ...
    fn test_custom_header_gets_indexed() {
        ...
        assert_eq!(
            encoder.header_table.dynamic_table.to_vec(),
            vec![Some((b"custom-key".to_vec(), b"custom-value".to_vec()))]
        );
